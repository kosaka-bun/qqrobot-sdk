package de.honoka.qqrobot.starter.framework.impl.tester.server;
...
import de.honoka.qqrobot.starter.framework.impl.tester.TesterFramework;
...
public class TesterServerConnection {
    ...
    public void onMessage(String messageStr) {
        ...
        switch(message.getType()) {
            case TesterMessageTypes.LOGIN:
                sendMessage(onLogin(message));
                break;
            case TesterMessageTypes.QUERY_ONLINE:
                sendMessage(queryOnline(message));
                break;
            case TesterMessageTypes.GROUP_MESSAGE:
                onGroupMessage(message);
                break;
            case TesterMessageTypes.PRIVATE_MESSAGE:
        ...
        for(TesterServerConnection connection : testerServer.getConnections()) {
            connection.sendMessage(
                new TesterMessage(null)
                    .setType(TesterMessageTypes.UESR_LOGOUT)
                    .setData(data)
            );
        }
    }
    ...
    private TesterMessage onLogin(TesterMessage message) {
        ...
        res.setType(TesterMessageTypes.LOGIN);
        //提取并检查消息中的成员
        ...
        testerServer.getExecutor().execute(() -> {
            for(TesterServerConnection connection : testerServer.getConnections()) {
                ...
                connection.sendMessage(
                    new TesterMessage(null)
                        .setType(TesterMessageTypes.NEW_USER_LOGIN)
    ...
    private void onGroupMessage(TesterMessage message) {
        ...
        sendMessage(
            new TesterMessage(message.getId())
                .setType(TesterMessageTypes.GROUP_MESSAGE_RESPONSE)
                .setData(resData)
        );
        //提醒其他用户
    ...
    private void onPrivateMessage(TesterMessage message) {
        ...
        sendMessage(
            new TesterMessage(message.getId())
                .setType(TesterMessageTypes.PRIVATE_MESSAGE_RESPONSE)
                .setData(resData)
        );
        //处理消息
        JSONArray content = message.getData().getJSONArray("content");
