package de.honoka.qqrobot.starter.component.util;
...
public class RobotImageUtils {
    ...
    @SneakyThrows
    public InputStream htmlToImage(String html, int width) {
    ...
    @SneakyThrows
    private String getTextImageHtml(String str) {
        URL textHtml = RobotImageUtils.class.getResource("/text.html");
     ...
     */
    private String forceWarp(String text, int lineLength) {
        //提取行
     ...
     * 指定图片大小，不限制每行字符数
     */
    public InputStream textToImageBySize(String text, int imageSize) {
     ...
     */
    public InputStream textToImageByLength(String text, int lineLength) {
        return htmlToImage(getTextImageHtml(forceWarp(text, lineLength)),
                lineLength * 53);
    }

    public InputStream textToImage(String text) {
