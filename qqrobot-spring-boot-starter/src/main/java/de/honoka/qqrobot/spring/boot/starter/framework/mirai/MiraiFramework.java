...
public class MiraiFramework extends Framework {
	...
	public void init() {
		...
		String deviceInfoPath = Path.of(FileUtils.getClasspath(),
				miraiProperties.getWorkDirectory(),
				"deviceInfo.json").toString();
		String logBase = Path.of(FileUtils.getClasspath(), miraiProperties
				.getWorkDirectory(), "log") + File.separator;
	 ...
	 * 为框架配置设置登录协议
	 */
	private void setProtocol(BotConfiguration conf, String protocolName) {
		...
		for(String part : parts) {
			...
			switch (actionCode.getAction()) {
				...
				case "image":
					...
					Image img;
					if(group == null) {
						img = getPrivateContact(qq).uploadImage(imgRes);
					} else {
						img = miraiApi.getGroup(group).uploadImage(imgRes);
					}
					builder.add(img);
					break;
				default:
	...
	public String transform(Object multiPartMsg) {
		...
		for(SingleMessage sm : miraiMultiPartMsg) {
			if(sm.getClass().equals(At.class))
				str += CC.at(((At) sm).getTarget());
			else
				str += sm.contentToString();
		}
		return str;
	}
	...
	public String getGroupName(Long group) {
		try {
			if(group == null)
				return "【私聊消息】";
			return miraiApi.getGroup(group).getName();
		} catch (Exception e) {
			return "未知";
		}
	}
	...
	public synchronized String getNickOrCard(Long group, long qq) {
		try {
			String nameCard = miraiApi.getGroup(group).get(qq).getNameCard();
			if(nameCard.trim().equals(""))	//群名片为空串，代表没有设置群名片
				nameCard = miraiApi.getGroup(group).get(qq).getNick();
			return nameCard;
		} catch (Exception e) {
	...
	public boolean isMuted(Long group) {
		try {
			return miraiApi.getGroup(group).getBotMuteRemaining() > 0;
		} catch (Exception e) {
	...
	public boolean sendTestMessageOnRelogin() {
		...
		try {
			receipt = miraiApi.getGroup(basicProperties.getDevelopingGroup())
